...
class MypyExternalAnnotator : ExternalAnnotator<Info, AnnotationResult>() {
    ...
    override fun doAnnotate(collectedInfo: Info?): AnnotationResult? {
        ...
        val lineRegex = Regex("""^(.*?):(\d+):(\d+):(?:(\d+):(\d+):)? (\w+): (\w.*?)(?: {2}\[([\w-]+)])?$""")
            ...
            if (line.startsWith(lookFor)) lineRegex.find(line)
                ?.let {
                    ResultItem(
                        ...
                        "${it.groups[7]!!.value}${it.groups[8]?.value?.let { code -> " [${code}]" }.orEmpty()}",
                        it.groups[8]?.value,
                    )
                }
            else null
    ...
    override fun apply(file: PsiFile, annotationResult: AnnotationResult?, holder: AnnotationHolder) {
        val baselines = doBaseline(file.virtualFile.path, file.project)
        baselines.forEach { item ->
            ...
            val element = (file.findElementAt(startOffset) ?: return@forEach)
                .let { if (it is LeafPsiElement && it.parent !is PyFunction) it.parent else it }
            if (element is PyFile) {
                return@forEach
            }
            val annotation = holder.newAnnotation(item.severity, item.message).range(element)
            annotation.newFix(SuppressIntention(item)).registerFix().enforcedTextAttributes(baselineTextAttributes)
            val name = file.findElementAt(startOffset)?.parentOfType<PyFunction>()?.qualifiedName
            if (name != null) {
        ...
        annotationResult?.items?.forEach { item ->
            for (baseline in baselines) {
                if (item.lineStart == baseline.lineStart
                    && item.columnStart == baseline.columnStart
                    && item.message == baseline.message.removePrefix("baselined: ")
                )
                    return@forEach
            }
            ...
            val annotation = holder.newAnnotation(item.severity, item.message).range(TextRange(startOffset, endOffset))
            if (item.code != null) {
                val parentFunction = file.findElementAt(startOffset)?.parentOfType<PyFunction>()
                val name = parentFunction?.qualifiedName
                if (name != null && startOffset <= (parentFunction.annotation?.endOffset
